...
int builtin_cmd(char **argv) 
{
	...
	if(!strcmp(argv[0], "quit"))	/* quit command */
		exit(0);
	else if(!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg")) {	/* fg or bg command */
...
void waitfg(pid_t pid)
{
	struct job_t *orig_jobpid = getjobpid(jobs, pid);	/* get pid of job to wait for */
...
void sigchld_handler(int sig) 
{
    ...
    while((child_pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) {
        sigprocmask(SIG_BLOCK, &mask_all, &mask_prev);	/* block signal which can affect our conduction*/
        if(WIFEXITED(status)) {	/* jobs that exits from progress normally */
            if(!deletejob(jobs, child_pid))
                printf("Job [%d] (%d) delete job error", pid2jid(child_pid), child_pid);
        }
        ...
        else if(WIFSTOPPED(status)) {	/* jobs that be stopped by some signals */
            getjobpid(jobs, child_pid)->state = ST;
        }

        else
            printf("Job [%d] (%d) terminated abnormally\n", pid2jid(child_pid), child_pid);
        sigprocmask(SIG_SETMASK, &mask_prev, NULL);
    }
